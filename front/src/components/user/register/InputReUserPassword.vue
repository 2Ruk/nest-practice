  ...
  <b-row align-h="center mt-1">
    <b-col cols="6">
      <b-form-group
          ...
          label-align="left"
          :label-for="inputId"
          description="비밀번호를 다시 입력해주십시오(4글자 이상)">
        <template #default>
          <b-input size="sm" id='user-id' type="text" v-model="userProp" placeholder="비밀번호를 다시 입력해주세요"/>
        </template>
      </b-form-group>
    </b-col>
  </b-row>
...
export default class InputReUserPassword extends Vue {
  @Prop() userProp: string = '';
  private label: string = 'RE:PW '
  private inputId: string = 'userPwRe'
  constructor() {
    super();
  }
